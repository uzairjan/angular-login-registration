...
export class AuthService {
  ...
  login(user: User) {
    return this.httpClient.post<any>(`${this.API_URL}/users/authenticate`, user)
      .subscribe((res: any) => {
  ...
  getUserProfile(id): Observable<any> {
    return this.httpClient.get(`${this.API_URL}/users/current?=${id}`, { headers: this.headers }).pipe(
      map((res: Response) => {
        return res || {}
      }),
      catchError(this.handleError)
